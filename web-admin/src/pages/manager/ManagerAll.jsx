...
import { CATEGORIES, STATUS, VISIBILITY, formatRelativeTime } from '../../utils/facility';
...
export default function ManagerAll() {
  ...
  const filteredThreads = useMemo(() => {
    return threads.filter((tc) => {
      ...
      // Status segregation (pending / ongoing / completed)
      ...
      // Sub-category check (private vs public)
      if (subCategory === 'private') {
        ...
        // merged - threads cannot be in merged here
        return false;
      }
  ...
  const filteredMerged = useMemo(() => {
    ...
    return mergedGroups.filter((group) => {
      ...
      if (activeTab === 'pending') {
        return false; // Merged groups are never pending in main flow
      } else if (activeTab === 'ongoing') {
