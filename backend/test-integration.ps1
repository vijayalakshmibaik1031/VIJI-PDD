...
Invoke-RestMethod "$base/employees/register" -Method POST -ContentType "application/json" -Body (@{ id = $empId; name = "Test Emp"; password = "Pass123!" } | ConvertTo-Json) | Out-Null
$loginRes = Invoke-RestMethod "$base/employees/login" -Method POST -ContentType "application/json" -Body (@{ userId = $empId; password = "Pass123!" } | ConvertTo-Json)
